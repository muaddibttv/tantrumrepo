...
from resources.lib.modules import cache, cleantitle, client, dom_parser2, log_utils
...
class source:
    ...
    def episode(self, url, imdb, tvdb, title, premiered, season, episode):
        try:
            ...
            try:
                ...
                r = [(i[0], i[1].split(':')[0], i[2]) for i in r if (cleantitle.get(i[1].split(':')[0])
                                                                     == cleantitle.get(url['tvshowtitle']) and i[2] == str(int(season)))]
                url = r[0][0]
            except Exception:
                pass
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            r = [(client.parseDOM(i, 'a', ret='href')[0], client.parseDOM(
                i, 'p', attrs={'class': 'server_servername'})[0]) for i in r]
            if r:
                for i in r:
                    try:
