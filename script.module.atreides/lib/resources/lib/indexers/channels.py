...
from resources.lib.modules import cleangenre, client, control, metacache, trakt, workers
...
class channels:
    def __init__(self):
        ...
        self.uk_dt = self.uk_datetime()
        self.systime = (self.uk_dt).strftime('%Y%m%d%H%M%S%f')
    ...
    def sky_list(self, num, channel, id):
        try:
            ...
            dt1 = (self.uk_dt).strftime('%Y-%m-%d')
            dt2 = int((self.uk_dt).strftime('%H'))
            if (dt2 < 6):
                dt2 = 0
    ...
    def items_list(self, i):
        try:
            ...
            if not mpaa:
                mpaa = '0'  # This should read 'mpaa = "fuckers"'
